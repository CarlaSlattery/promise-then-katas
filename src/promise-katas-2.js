...
const dog = async () => {
  ...
  const naughtiestDog = response.data.dogs.reduce((previousDog, currentDog) =>
    previousDog.naughty > currentDog.naughty ? previousDog : currentDog
  );
...
const joke = async () => {
  const questionResponse = await fetch("jokes", "question");
  const answerResponse = await fetch("jokes");
  return {
